...
1. /superpowers:systematic-debugging 测试一下，release.sh编译构建后，使用release中的cli打开opencode，然后使用cli输入"你是谁？"， 并出发回车发送。这个过程每一步都截图分析，保存到@release_test/${{yyyy-mm-dd hh-mm-ss}}文件夹下。最后使用`cli-box close <id>`关闭沙箱，并使用`cli-box list`进行验证
