...
pub fn build_router(state: Arc<Mutex<AppState>>) -> Router {
    ...
    Router::new()
        ...
        .route("/box/info", get(sandbox_info_handler))
        .route("/box/pending-cli", get(pending_cli_handler))
...
const SANDBOX_WINDOW_REQUIRED: &str =
    "Sandbox window not available. Build and run the Tauri app first, or use `cli-box start`.";
