...
const LOG_BASE: &str = ".cli-box/logs";
...
/// Base log directory: `~/.cli-box/logs/`
...
/// Today's log directory: `~/.cli-box/logs/YYYY-MM-DD/`
...
pub fn server_log_path() -> PathBuf {
    today_log_dir().join("cli-box-server.log")
}

/// Get the CLI log path.
pub fn cli_log_path() -> PathBuf {
    log_base_dir().join(format!("cli-box-cli.log.{}", today_str()))
}
...
pub fn cleanup_old_logs() {
    ...
    for entry in entries.flatten() {
        ...
        // Match rolling CLI logs: cli-box-cli.log.YYYY-MM-DD
        if let Some(suffix) = name.strip_prefix("cli-box-cli.log.") {
            if suffix.len() == 10 {
...
///
/// Creates:
/// - `~/.cli-box/logs/{date}/{sandbox_id}.log` (sandbox-specific)
/// - `~/.cli-box/logs/{date}/cli-box-server.log` (shared server)
///
...
pub fn init_sandbox_logging(sandbox_id: &str) -> (WorkerGuard, WorkerGuard) {
    ...
    let server_appender = RollingFileAppender::builder()
        .rotation(tracing_appender::rolling::Rotation::DAILY)
        .filename_prefix("cli-box-server.log")
        .max_log_files(MAX_LOG_DAYS as usize)
        .build(&dir)
...
///
/// Creates `~/.cli-box/logs/cli-box-cli.log.{YYYY-MM-DD}`.
...
pub fn init_cli_logging() -> WorkerGuard {
    ...
    let appender = RollingFileAppender::builder()
        .rotation(tracing_appender::rolling::Rotation::DAILY)
        .filename_prefix("cli-box-cli.log")
        .max_log_files(MAX_LOG_DAYS as usize)
        .build(&dir)
